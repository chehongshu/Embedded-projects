...
{
	TIM5->SR&=~(1<<0);	//清除中断标志位 
	TIM5->ARR=0XFFFF;	//将重装载值设置到最大
	TIM5->CNT=0;		//清空定时器的CNT
...
{
	if(TIM5->SR&0X0001)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=TIM5->CNT;
...
void Timer4_Init(u16 arr,u16 psc)
{
	...
	TIM_ITConfig( TIM5, TIM_IT_Update|TIM_IT_Trigger, ENABLE );//TIM5 允许更新，触发中断
