...
void usart3_init(u32 bound)
{
	  ...
	  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);	//使能USART1，GPIOA时钟  zhuying 
...
//	  NVIC_Configuration();
      ...
      USART_Init(USART3, &USART_InitStructure); //初始化串口//初始化UART1
	  USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);//使能中断 接收中断 接收标志RXNE
...
{
  USART_SendData(USART_x, data);//  send_data是发送字符类型的 
  while(USART_GetFlagStatus(USART_x, USART_FLAG_TXE) == RESET){}////等待发送结
}
/****************************
